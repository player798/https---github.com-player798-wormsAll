...
function OnlineBenutzerLobby() {
    axios.get('http://localhost:8080/lobby/onlineUsers')
        .then(response => {
            ...
            var onlineSpielerAusgabeHTML = document.getElementById('onlineSpielerAusgabe');
       
            // Überprüfe, ob der Container noch leer ist, um "Online:" nur einmal hinzuzufügen
       if (onlineSpielerAusgabeHTML.innerHTML === '') {
        onlineSpielerAusgabeHTML.innerHTML = "Online:<br>";
    }

            onlineBenutzerListe.forEach(function (benutzer) {
                var einzelnesBenutzerDiv = document.createElement('div');
                einzelnesBenutzerDiv.textContent = benutzer.username;
                einzelnesBenutzerDiv.dataset.userId = benutzer.id; // Setze die Benutzer-ID als Datensatzattribut
                einzelnesBenutzerDiv.classList.add('einzelnerBenutzerDiv'); // Füge der Klasse 'chatBenutzer' hinzu
                onlineSpielerAusgabeHTML.appendChild(einzelnesBenutzerDiv); // Füge das Benutzer-Element dem Container hinzu
});
  ...
  // Füge einen Eventlistener hinzu, um auf Klicks auf Benutzer zu reagieren
  benutzernamenElement.addEventListener('click', function(event) {
    sendeSpieleEinladung(benutzer.id); // Einladung senden, wenn auf den Benutzer geklickt wird
});
